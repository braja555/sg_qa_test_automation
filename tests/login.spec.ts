...
describe("Login Test", () => {
  ...
  test("Validation for Empty Email and Password Fields", async () => {
    logger.info('Execution started: Validation for empty email and password fields');
    ...
    logger.info('Execution ended: Validation for empty email and password fields');
  });
  
  test("Validation for Invalid Email Address", async () => {
    logger.info('Execution started: Validation for invalid email address');
    await loginPage.loginToApplication(testData.invalid_email, testData.password);
    await validateAlertErrorMessage();
    emailErrorLocator = page.getByTestId(loginPageSelectors.errorEmail);
    await expect(emailErrorLocator).toHaveText("User not found or password doesn't not match");
    logger.info('Execution ended: Validation for invalid email address');
  });
  ...
  test("Validation for Invalid Password", async () => {
    ...
    await validateAlertErrorMessage();
    passwordErrorLocator = page.getByTestId(loginPageSelectors.errorPassword);
    await expect(passwordErrorLocator).toHaveText("User not found or password doesn't not match");
    logger.info('Execution ended: Validation for invalid password');
  });
  
  test("Validation for Unregistered User", async () => {
    logger.info('Execution started: Validation for unregistered user');
    await loginPage.loginToApplication(testData.unregistered_email, testData.password);
    await validateAlertErrorMessage();
    emailErrorLocator = page.getByTestId(loginPageSelectors.errorEmail);
    await expect(emailErrorLocator).toHaveText("User not found or password doesn't not match");
    logger.info('Execution ended: Validation for unregistered user');
  });
})
